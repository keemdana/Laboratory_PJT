...
var $modalDiv;
                ...
                if (typeof json === "object") {
                    ...
                    if(processList > 0){
                    	...
                    	for(var i = 0; i < processList; i++){
                    		...
                    		html += "<table class='basic pr prDiv"+tbDeptCnt+"' id='pr' name='prDiv'>";
                    		...
                    		if(status == "VOC03"){
                    			...
                    			html +=	"<th><span class='ui_icon required'></span>처리방안</th> <td> <select id='prcPlanType"+tbDeptCnt+"' name='prcPlanType"+tbDeptCnt+"' class='form-control input-sm' data-type='search'></select> </td>";
                        		html +=	"<th><span class='ui_icon required'></span>대책완료예정일</th> <td> <input type='text' id='processPlanDate"+tbDeptCnt+"' name='processPlanDate"+tbDeptCnt+"' class='form-control' style='width: 100%;'> </td>";
                        		html +=	"<th><span class='ui_icon required'></span>처리현황</th> <td> <select id='prcStatus"+tbDeptCnt+"' name='prcStatus"+tbDeptCnt+"' onchange='chagePrcStatus(\""+isEmpty(json.process[tbDeptCnt].prcStatus)+"\", \""+tbDeptCnt+"\")' onFocus='focus()' class='form-control input-sm' data-type='search'></select> </td>";
                        		html +=	"</tr>";
                    	    ...
                    	    if(status != "VOC02" || status != "VOC03"){
                    	    	html += "<div class='conButtonlist'>"
                    	    	html += "<div class='right'>"
                    	    	html +=	"<button type='button' class='btn btn-primary' onclick='doProSubmit(\""+json.process[tbDeptCnt].vocNo+"\", \""+json.process[tbDeptCnt].seq+"\", \""+json.process[tbDeptCnt].deptCd+"\");''><i class='fa fa-check'></i>처리등록</button>"
                    	    	html += "</div>"
                    	    	html += "</div>"
                    	    	
                    	    	/* $("#btnProSave"+tbDeptCnt).off();
                    			$("#btnProSave"+tbDeptCnt).on("click", function() {
                    				doProSubmit(tbDeptCnt);
                    			}); */
                    	    }
                        	...
                        	//권한 체크
                        	if(status == "VOC03"){
                        		//처리부서 값 조회해서 그 부서의 팀장, 그룹장 조회
            					var deptData = {"vocNo" : d_vocNo, "deptCd" : json.process[tbDeptCnt].deptCd}
                        		var data = $.extend({}, deptData, jsonData);
            
                        		paragonCmm.callAjax("/voc/deptJang/json", data, function (json) {
                        			var groupJangId = isEmpty(json.dept.userId);
                        			if($("#loginUserId").val() != groupJangId){
                        				//해당 부서의 장이 아닐 경우 비활성화
                        				$("#processUserId"+tbDeptCnt+"").attr("disabled", true);
    	                			}
                        		});
                        		
                        		/* if($("#loginUserId").val() != isEmpty(json.process[tbDeptCnt].prcUserId)){
	                				$(".prDiv"+tbDeptCnt).addClass("disabledbutton");
	                			} */
                        	}else if(status == "VOC04"){
	                        	if($("#loginUserId").val() != isEmpty(json.process[tbDeptCnt].prcUserId)){
	                				$(".prDiv"+tbDeptCnt).addClass("disabledbutton");
	                			}
                			}
                    ...
                    */
            		...
            		//권한 체크
        			if($("#loginUserId").val() != master.createUserId){
        				$("#mstDiv").addClass("disabledbutton");
        			}
		...
		*/
		...
		/*
			* 처리현황 정보 저장
			  -> 권한자 : 처리 담당자		
		*/
		/* var doProSubmit = function(tbDeptCnt) {
			var msg = paragonCmm.getLang("M.ALERT_SUBMIT", "L.저장");
			if (!doValidation("PRO", tbDeptCnt)) {
				return false;
			}
	...
	function chagePrcStatus(prcStatus, seq){
		...
		if(seq > 0){
			var prevSeq = seq - 1;
			var bfPrcStatus = $("#prcStatus"+prevSeq+" option:selected").val();	//순차 시 앞 단계 처리현황
			var crPrcStatus = $("#prcStatus"+seq+" option:selected").val();	//순차 시 현 단계 처리현황
			
			if(crPrcStatus == "101"){	//현 단계의 값을 현상파악완료로 변경하려고 한다면 앞 단계의 처리현황이 현상파악완료인지를 확인한다.
				if(bfPrcStatus != "101"){	//현상파악완료가 아닐 경우
	...
	//처리현황 버튼 이벤트
	function doProSubmit(vocNo, seq, deptCd){
		var msg = paragonCmm.getLang("M.ALERT_SUBMIT", "L.저장");
		if (!doProValidation(seq)) {
			return false;
		}
		...
		confirm(msg, function(r) {
			if (r) {
				...
				var dataObj = {};
				//for(var i = 0; i < $("#processDiv").children("#pr").length; i++){
					var deptCd = $("#processDept"+seq).val();
					var prcPlanType = $("#prcPlanType"+seq).val();
					var prcPlanDate = $("#processPlanDate"+seq).val();
					var prcStatus = $("#prcStatus"+seq).val();
					var ecrNo = $("#processEcrNo"+seq).val();
					...
					paragonCmm.setEditorSubmit("");
					var contents = $("[name='prDiv']").find("textarea[name=processContents"+seq+"]").val();
					if(contents != ""){
						dataObj.prcContents = contents.replace(/\"/gi, "'");	
					}else{
						dataObj.prcContents = "";
					...
					var atchFileId = data._attachFileRelUid[seq];
					dataObj.atchFileId = atchFileId;
	...
	//처리현황은 필수값 체크 따로한다.
	function doProValidation (seq){
		if ($("#prcPlanType"+seq+" option:selected").val() == "") {
			alert("처리방안을 선택해주세요.", function() {
				$("input:text[name='prcPlanType"+seq+"']", $("#vocWriteForm")).focus();
			});
			return false;
		}
		
		if ($("#processPlanDate"+seq).val() == "") {
			alert("대책완료 예정일을 선택해주세요.", function() {
				$("input:text[name='processPlanDate"+seq+"']", $("#vocWriteForm")).focus();
			});
			return false;
		}
		
		if ($("#prcStatus"+seq+" option:selected").val() == "") {
			alert("처리현황을 선택해주세요.", function() {
				$("input:text[name='prcStatus"+seq+"']", $("#vocWriteForm")).focus();
			});
			return false;
		}
